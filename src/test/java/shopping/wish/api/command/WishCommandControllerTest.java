...
class WishCommandControllerTest {
    ...
    @Test
    @DisplayName("유효한 토큰으로 위시리스트 상품을 삭제할 수 있다")
    void test04() throws Exception {
        ...
        // assert
        Wish deletedWish = wishRepository.findById(wish.getId()).orElseThrow();
        assertThat(deletedWish.isDeleted()).isTrue();
    }

    @Test
    @DisplayName("토큰 없이 위시리스트 상품을 삭제하면 401을 반환한다")
    ...
    @Test
    @DisplayName("존재하지 않는 위시리스트를 삭제하면 400을 반환한다")
    void test06() throws Exception {
        ...
        mockMvc.perform(delete("/api/wishes/999")
                        .header(HttpHeaders.AUTHORIZATION, "Bearer " + token))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.message").value("존재하지 않는 위시리스트입니다."));
    }
}
