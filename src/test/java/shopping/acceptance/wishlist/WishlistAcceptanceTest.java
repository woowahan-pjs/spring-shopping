...
public class WishlistAcceptanceTest extends AcceptanceTest {
    ...
    void registerWishList() {
        ...
        final long 상품 = ProductAcceptanceSteps.상품등록됨(상점, 서브카테고리, ProductFixture.NAME, ProductFixture.AMOUNT, ProductFixture.THUMBNAIL_IMAGE_URL, ProductFixture.DETAILED_IMAGE_URLS, sellerAccessToken);
    ...
    @DisplayName("위시 리스트가 등록되어 있다면 사용자는 위시 리스트를 조회할 수 있다")
    @Test
    void findAllWishList() {
        ...
        WishListAcceptanceSteps.등록됨(상품, customerAccessToken);
        final ExtractableResponse<Response> response = WishListAcceptanceSteps.목록조회(customerAccessToken);
        WishListAcceptanceSteps.validateList(response);
    }
}
